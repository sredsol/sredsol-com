...
- **Preserve cross-cutting features.** i18n (English default, locale routing ready
  under `src/pages/[locale]/`), SEO/OG/RSS/sitemap, Pagefind search, and the Starlight
  docs.
- **Content lives in EmDash.** Blog posts and pages are sourced from the CMS — see
  [Content Management](/docs/guides/content-management/). Don't hardcode page copy that
...
```bash
pnpm build          # full server build
pnpm lint           # eslint + stylelint + type-check + KPIs + i18n + secrets
