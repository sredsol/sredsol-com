...
| GridBackground | `ui/layout/GridBackground/GridBackground.astro` | `variant` (grid\|dots\|crosshairs\|isometric), `glow`, `glowPosition`, `mask` | Spatial grid canvas backdrop with ambient glow. |
| NodeIndicator | `ui/data-display/NodeIndicator/NodeIndicator.astro` | `label`, `code`, `domain`, `status`, `size`, `interactive` | Computational concept vertex and module anchor. |
| SignalBadge | `ui/data-display/SignalBadge/SignalBadge.astro` | `protocol`, `status`, `latency`, `variant` (pill\|terminal\|minimal) | Live telemetry, protocol & status signal indicator. |
| ScopeGauge | `ui/data-display/ScopeGauge/ScopeGauge.astro` | `variant` (waveform\|dial\|frequency\|matrix), `label`, `value`, `unit`, `channel` | SVG/CSS oscilloscope waveform and dial measurement gauge. |
| SystemLayer | `ui/layout/SystemLayer/SystemLayer.astro` | `step`, `code`, `title`, `domain`, `description`, `technologies`, `isLast` | Architecture stack step with vertical connector wire. |
| ExplorationCard | `ui/data-display/ExplorationCard/ExplorationCard.astro` | `title`, `slug`, `domain`, `domainLabel`, `status`, `description`, `sceneId`, `technologies`, `href` | Studio exploration showcase card with scene slot. |
| NodeCanvas | `interactive/NodeCanvas.astro` | `class`, `nodeCount` | High-performance interactive pointer-reactive node field canvas. |
| InstrumentScope | `interactive/InstrumentScope.astro` | `class`, `initialFreq`, `initialAmp` | Interactive real-time waveform scope with frequency/amplitude bench controls. |
| StudioPreview | `interactive/StudioPreview.astro` | `sceneId`, `class` | Micro-scene preview canvas for Exploration Cards. |

| Checkbox | `ui/form/Checkbox/Checkbox.astro` | `id`, `name`, `label`, `checked` | Accessible checkbox. |
| Switch | `ui/form/Switch/Switch.astro` | `id`, `name`, `label`, `checked` | Accessible toggle switch. |
...
| HeroNodeField | `sections/HeroNodeField.astro` | `locale`, `class` |
| DomainGrid | `sections/DomainGrid.astro` | `locale`, `class` |
| ExplorationsGrid | `sections/ExplorationsGrid.astro` | `title`, `eyebrow`, `limit`, `showViewAll`, `locale`, `class` |
| SystemsLayerDiagram | `sections/SystemsLayerDiagram.astro` | `locale`, `class` |
| ThinkingDigest | `sections/ThinkingDigest.astro` | `limit`, `locale`, `class` |
| LabCTA | `sections/LabCTA.astro` | `locale`, `class` |
