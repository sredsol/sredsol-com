...
## Studio Scope Window & Waveform

Embedded oscilloscope waveform or telemetry dial to visualize measurement without client JS graphing overhead.
...
Rules: stroked with `--primary`, background grid derived from `--foreground` color-mix, monospace readout numbers.

## Architecture Layer & Pipeline Connector

Vertical pipeline connector with numbered node indicator and sequential wire for system stacks.
...
<SystemLayer
  ...
  description="Mathematical foundations and dynamic simulation."
  technologies={["Discrete Differential Geometry", "Topology", "WASM"]}
/>
```
